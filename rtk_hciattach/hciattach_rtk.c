...
#define RTL_FW_MATCH_HCI_REV    (1 << 1)
struct patch_info {
...
static struct patch_info patch_table[] = {
	 ...
	 * RTL8761ATV
	 * */
	{ 0, 0, ROM_LMP_8723a, ROM_LMP_8723a, 0,
		"rtl8723a_fw", "rtl8723a_config", "RTL8723AS"},
	{ 0, 0, ROM_LMP_8723b, ROM_LMP_8723b, 0,
		"rtl8723b_fw", "rtl8723b_config", "RTL8723BS"},
	{ 0, 0, ROM_LMP_8821a, ROM_LMP_8821a, 0,
		"rtl8821a_fw", "rtl8821a_config", "RTL8821AS"},
	{ 0, 0, ROM_LMP_8761a, ROM_LMP_8761a, 0,
	...
	{ RTL_FW_MATCH_CHIP_TYPE, CHIP_8703AS,
		ROM_LMP_8723b, ROM_LMP_8723b, 0,
		"rtl8703a_fw", "rtl8703a_config", "RTL8703AS"},
#endif
	{ 0, 0, ROM_LMP_8822b, ROM_LMP_8822b, 0,
...
static void hci_event_cmd_complete(struct sk_buff *skb)
{
	...
	case HCI_CMD_READ_LOCAL_VERISION:
		...
		if (0 == status) {
			...
			RS_DBG("HCI Version 0x%02x, HCI Revision 0x%04x, "
			       "LMP Subversion 0x%04x", rtk_hw_cfg.hci_ver,
			       rtk_hw_cfg.hci_rev, rtk_hw_cfg.lmp_subver);
		} else {
...
void rtk_get_final_patch(int fd, int proto)
{
	...
	if (rtl->lmp_subver != ROM_LMP_8703b) {
		...
		/* if (rtk_hw_cfg.patch_ent->prod_id != project_id[proj_id]) {
		 * 	RS_ERR("prod_id %04x, version %04x from firmware "
		 * 	       "project_id[%u], mismatch\n",
		 * 	       rtk_hw_cfg.patch_ent->prod_id,
...
static int rtk_config(int fd, int proto, int speed, struct termios *ti)
{
	...
	if (proto == HCI_UART_3WIRE) {
		...
		rtk_get_eversion(fd);
		RS_INFO("LMP subversion 0x%04x", rtk_hw_cfg.lmp_subver);
		RS_INFO("rtk_hw_cfg.eversion = %d", rtk_hw_cfg.eversion);
	} else {
		/* RTL8761AT H4 */
...
int rtk_post(int fd, int proto, struct termios *ti)
{
	...
	fprintf(stderr, "--shinq--rtk_hw_cfg.final_speed=%d.\n",rtk_hw_cfg.final_speed);
	if (rtk_hw_cfg.final_speed)
		return set_speed(fd, ti, rtk_hw_cfg.final_speed);
	fprintf(stderr, "--shinq--rtk_post END.\n");
	return 0;
}
